<x-app title=" | LPJ">
    <div class="card-header py-2">
        <h5 class="m-0 font-weight-bold text-dark" style="text-align:center; font-size: 25px"> INFO DAFTAR JADWAL LPJ
        </h5>
    </div>
    <br>
    <x-template.button.back-button url="admin/pengajuancso/barulpj" />
    <div class="card">
        <div class="card-body">
            ...
            <div class="row">
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" readonly class="form-control form-control-plaintext" id="jam_pengajuan"
                        style="text-align: center" value="{{ $lpj->jam_pengajuan }} WIB">
                </div>
                <div class="col-md-4">
                    ...
                    <input type="text" readonly class="form-control form-control-plaintext" id="jam_selesai"
                        style="text-align: center" value="{{ $lpj->jam_selesai }} WIB">
                </div>
            ...
            <div class="col-md-12">
                <div class="form-group d-flex justify-content-end">
                    @if ($lpj->status == 'DiProses...')
                        <form id="acceptForm" action="{{ url('admin/pengajuancso/barulpj', $lpj->id) }}/accept"
                            method="post">
                            @csrf
                        ...
                        <form id="rejectForm" action="{{ url('admin/pengajuancso/barulpj', $lpj->id) }}/reject"
                            method="post">
                            @csrf
            ...
            <div class="modal-body">
                <form id="balasanForm" action="" method="post" style="display: none;">
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control" id="balasan_wa" name="balasan_wa" rows="5">*Hallo*, Perkenalkan saya *Agung* Bagian Admin KPPN Ketapang. Yang terhormat Kepada *{{ $lpj->satker->nama_satker }}* dengan Kode Satker *{{ $lpj->satker->kode_satker }}* untuk pertemuan jadwal konsultasi LPJ dengan pegawai KPPN Ketapang bagian *{{ $lpj->pegawai->jabatan }}* pada tanggal *{{ $lpj->tanggal_pengajuan }}* pada pukul *{{ $lpj->jam_pengajuan }} WIB* hingga pukul *{{ $lpj->jam_selesai }} WIB*. Pengajuan Jadwal Pertemuan Konsultasi *Di Terima*. Silahkan datang ke KPPN Ketapang dan bertemu dengan *{{ $lpj->pegawai->nama_pegawai }}* *10 Menit* sebelum Jam *{{ $lpj->jam_pengajuan }} WIB* dimulai. Sekian atas perhatian kami, ucapkan Terima Kasih.</textarea>
                    </div>
                    <div class="row">
                        <div class="col-md-12">
                ...
                <form id="rejectFormModal" action="" method="post" style="display: none;">
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control" id="balasan_wa" name="balasan_wa" rows="5">
                            *Hallo*, Perkenalkan saya *Agung*, bagian Admin KPPN Ketapang. Mohon Maaf Atas Pengajuan Jadwal Konsultasi LPJ dari Satker {{ $lpj->satker->nama_satker }} dengan Kode Satker {{ $lpj->satker->kode_satker }} dengan pegawai kami *{{ $lpj->pegawai->nama_pegawai }}* dengan jabatan *{{ $lpj->pegawai->jabatan }}*. Yang ingin bertemu pada tanggal *{{ $lpj->tanggal_pengajuan }}* pada pukul *{{ $lpj->jam_pengajuan }} WIB*, hingga *{{ $lpj->jam_selesai }} WIB* *DI Tolak*. Di karena pegawai tersebut sudah ada jadwal pertemuan konsultasi dengan Satker lain. Silahkan untuk pengajuan Jadwal ulang kembali. Dengan melihat di Kelander bagian Dashboard dengan Mengklick tanggal dikalender untuk melihat jadwal CSO dan CSK KPPN Ketapang. Sekian Kami Ucapkan Terima Kasih
                        </textarea>
                    </div>
                    <div class="row">
                        <div class="col-md-12">
...
<script>
    function showModal(action) {
        ...
        var acceptAction = '{{ url('admin/pengajuancso/barulpj', $lpj->id) }}/accept';
        var rejectAction = '{{ url('admin/pengajuancso/barulpj', $lpj->id) }}/reject';
